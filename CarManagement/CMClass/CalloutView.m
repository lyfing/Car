...
-(void) prepareFramSize;
-(void) prepareOffset;
-(CGFloat) relativeParentXPosition;
-(void) adjustMapRegionIfNeed;
@end
...
@synthesize contentWeight = _contentWeight;
@synthesize CalloutDelegate = _CalloutDelegate;
...
- (void)dealloc{
    ...
    [self.parentAnnotationView release];
    [self.mapView release];
    [self.contentView release];
    [self.CalloutDelegate release];
...
-(void) tapped{
    [self.CalloutDelegate tapAnnotationView];
}
...
-(void) prepareFramSize{
    ...
    frame.size = CGSizeMake(self.contentWeight, height);
    self.frame = frame;
    //    NSLog(@"self.mapView.frame.size.width:%f",self.mapView.frame.size.width);
    //    NSLog(@"self.frame:%f",frame.size.width);
}

-(void) prepareContentFrame{
    CGRect contenFrame = CGRectMake(self.bounds.origin.x+10, self.bounds.origin.y+3, self.contentWeight, self.contentHeight);
    self.contentView.frame = contenFrame;
    //    NSLog(@"content width :%f",self.contentView.frame.size.width);
}
...
-(void) adjustMapRegionIfNeed{
    ...
    if([self relativeParentXPosition] < self.frame.size.width - 38){
    ...
    }else if(pixelsFromTopOfMapView < 10){
...
- (CGFloat)relativeParentXPosition {
    ...
    return parentOrigin.x -(self.mapView.frame.size.width/2-self.contentWeight/2)+ self.offsetFromParent.x;
}
